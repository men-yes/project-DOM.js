...
const highestScoreTTT = localStorage.getItem("highestScoreTTT") || 0;
if (local) {
  const user = JSON.parse(local); // הפוך את המחרוזת לאובייקט
  const name = user.name;
  const scoreTTT = user.scoreTTT || 0; // ניקוד התחלתי 0 אם לא קיים
  israel.innerText = `Welcome ${name}. Your Tic Tac Toe score: ${scoreTTT}. Highest score: ${highestScoreTTT}`;
} else {
...
function btnClick() {
  ...
  if (chekIfWin()) {
    ...
    if (local) {
      ...
      user.scoreTTT = (user.scoreTTT || 0) + 1; // עדכון ניקוד
      localStorage.setItem("currentUser", JSON.stringify(user)); // שמירה ב-localStorage
      israel.innerText =`Welcome ${user.name}. Your Tic Tac Toe score: ${user.scoreTTT}. Highest score: ${highestScoreTTT}`;
      // בדיקה אם הניקוד הנוכחי הוא הגבוה ביותר
      if (user.scoreTTT > highestScoreTTT) {
        localStorage.setItem("highestScoreTTT", user.scoreTTT);
